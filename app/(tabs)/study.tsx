  ...
  return (
    <View style={[styles.root, { paddingTop: insets.top, backgroundColor: colors.background }]}>
      ...
      <View style={styles.header}>
        ...
        <Pressable style={[styles.addBtn, { backgroundColor: colors.accent }]} onPress={() => setDomainModal(true)}>
      ...
      {loading ? (
        ...
        <View style={styles.empty}>
          <Book size={52} color={colors.textDim} strokeWidth={1.5} />
          <Text style={[styles.emptyTitle, { color: colors.text }]}>Create your Study Domain</Text>
          <Text style={[styles.emptySubtitle, { color: colors.textMuted }]}>Organize subjects, resources, and track study hours</Text>
          <PrimaryButton title="Add Domain" onPress={() => setDomainModal(true)} style={{ marginTop: Spacing.lg }} />
        </View>
      ) : (
                      ...
                      return (
                        <View key={subject.id} style={[styles.subjectCard, { backgroundColor: colors.surfaceLight }]}>
                          <Pressable style={styles.subjectHeader} onPress={() => setExpandedSubject(isSubExpanded ? null : subject.id)}>
                            ...
                            <Text style={[styles.subjectMeta, { color: colors.textMuted }]}>{subject.study_hours ?? 0}h · {subject.resources?.length ?? 0}</Text>
...
const styles = StyleSheet.create({
  ...
  domainCard: { marginBottom: Spacing.md, overflow: 'hidden' },
  ...
  subjectName: { flex: 1, fontSize: Typography.sizes.sm, fontWeight: Typography.weights.semibold },
  subjectMeta: { fontSize: Typography.sizes.xs },
